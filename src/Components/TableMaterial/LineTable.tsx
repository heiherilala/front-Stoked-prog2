...
import { JobOffer, material } from '../../interfaces';
...
export const LigneList:React.FC<props> = (props) => {
    ...
    return (
        <>
            ...
            <tr id={""+item.name} key={item.idMaterial}>
                    <td>{item.name}</td>
                    <td>{item.unit}</td>
            ...
            <ConfirmDispo
                ...
                function = {()=>{
                    postPutDeletRequest("/offers",null,item.idMaterial,false,true,()=>finishLoadingt(),()=>()=>{setActivModif(false)},props.token);
                }
            }
            />:<></>}
            ...
            {activModifPut?<div className='fonds3'>
                <Modal show={true} onHide={()=>setActivModifPut(false)}>
                    <FormulaireMaterial changObject={item} finishFunction={()=>{setActivModifPut(false);props.actualisationAllData()}} />
                </Modal>
            </div>:<></>}
