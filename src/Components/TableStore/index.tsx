...
export const Tablestore: React.FC<props> = (props) => {
  ...
  const [dataList, setDataList] = useState<store[]>([]);
  ...
  useEffect(() => {
    setActivLoading(true)
    axiosGetWithPage("/stores",page,valuNumbur,myToken,setDataList,()=>{setActivLoading(false)},null)
  ...
  return (
    <div className="d-flex flex-column mb-3 ForcedFisplayColone marging-top">
      ...
      <Modal show={true} onHide={()=>setActivUpdatePost(false)}>
          <FormulaireStore changObject={undefined} finishFunction={()=>{setActivUpdatePost(false);actualisationAllData()}} />
      </Modal>
