...
export const LigneList:React.FC<props> = (props) => {
    ...
    return (
        <>
            ...
            <tr id={""+item.name} key={item.idStore}>
                    <td>{item.name}</td>
                    <td>{item.place}</td>
                    <td>{item.booleanFull}</td>
                    <td>{item.actualVolumeM3}</td>
                    <td>{item.actualWeigthKg}</td>
                {((props.mofif))?
                    <td>
            ...
            <ConfirmDispo
                ...
                function = {()=>{
                    postPutDeletRequest("/offers",null,item.idStore,false,true,()=>finishLoadingt(),()=>()=>{setActivModif(false)},props.token);
                }
            }
            />:<></>}
            ...
            {activModifPut?<div className='fonds3'>
                <Modal show={true} onHide={()=>setActivModifPut(false)}>
                    <FormulaireStore changObject={item} finishFunction={()=>{setActivModifPut(false);props.actualisationAllData()}} />
                </Modal>
            </div>:<></>}
