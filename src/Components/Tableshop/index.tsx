...
export const Tableshop: React.FC<props> = (props) => {
  ...
  const [dataList, setDataList] = useState<shop[]>([]);
  ...
  useEffect(() => {
    setActivLoading(true)
    axiosGetWithPage("/shops",page,valuNumbur,myToken,setDataList,()=>{setActivLoading(false)},null)
  ...
  return (
    <div className="d-flex flex-column mb-3 ForcedFisplayColone marging-top">
      ...
      <Modal show={true} onHide={()=>setActivUpdatePost(false)}>
          <FormulaireShop changObject={undefined} finishFunction={()=>{setActivUpdatePost(false);actualisationAllData()}} />
      </Modal>
